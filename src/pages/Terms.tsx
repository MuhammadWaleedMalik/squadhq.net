...
import { BookOpen, Globe, Lock, Archive } from 'lucide-react';
...
const Terms: React.FC = () => {
  ...
  const sectionIcons = [
    ...
    <Archive size={30} className="text-[${colors.primaryColor1}]" />,
  ];
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: colors.backgroundLight }}>
      ...
      <section 
        ...
        style={{ 
          backgroundImage: `linear-gradient(rgba(0,0,0,0.65), rgba(0,0,0,0.65)), url('/images/archaeology-timeline-bg.jpg')`
        }}
      >
        <motion.div
      ...
      <section className="py-24 px-6 relative">
        <div className="max-w-4xl mx-auto relative">
          ...
          >
            Terms Governing {websiteInfo.name}
          </motion.h2>
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <p className="text-base md:text-lg font-medium" style={{ color: colors.textPrimary }}>
                {currentLanguage.code === 'ja'
                  ? '利用規約に同意することで、{websiteName}のサービスを利用できます。'
                  : currentLanguage.code === 'zh'
                  ? '同意我们的服务条款以使用{websiteName}的服务。'
                  : `Agree to our Terms of Service to explore ${websiteInfo.name}'s historical and archaeological resources.`}
              </p>
